...
def read_input(n):
    """
    Modified from read.py to include a step count tracker for the agent
    """
    path = 'input.txt'
    ...
    if os.path.exists(path):
        with open(path, 'r') as f:
            lines = f.readlines()
            step_count = int(lines[0]) + 1

    if start_new_game(board):
        step_count = 1

    with open(path, 'w') as f:
        f.write(str(step_count))

    return piece_type, previous_board, board, step_count


def start_new_game(board):
    """
    Detect if a new game has started in order to reset the step counter for the agent
    """
